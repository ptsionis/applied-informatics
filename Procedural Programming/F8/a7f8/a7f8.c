...
void print_data(int num_rentals, car_rentalT rentals[], float total, best_carT best)
{
    ...
    printf("\n");
    for (int i=0; i<num_rentals; i++) {
        printf("%-7d %-20s %-10s %-5d %-5d %-6.2f %-8.2f\n", rentals[i].number, rentals[i].name, rentals[i].type,
               rentals[i].cc, rentals[i].days, rentals[i].daily_rate, rentals[i].total_charge);
    }
    for (int i=0; i<73; i++) printf("-");
    printf("\n");
    printf("%58s %-8.2f\n", "Total", total);
    printf("Best car: %s %d rented for %.2f Euros.", best.type, best.cc, best.amount);
}
